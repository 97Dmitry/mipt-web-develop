...
- при создании заказа сервис повторно проверяет остаток в `product-service` через `/internal/*` с `X-Internal-Token`
...
- admin endpoint `/admin/orders*` требуют Bearer JWT, выданный `admin-panel-service`
